...
if (!empty($_POST['submit'])) {
    ...
    if (count($t)!=count(explode("\n", $input))) {
        $error = 'The number of lines in the output (translation) must match the number of lines in the input.';
    }

    // echo count($t).' '.count(explode("\n",$input));

    if ($error) {
        echo '<p style="color: #a00; font-weight: bold;">'.$error.'</p>';
    } else {
        $index = 0;
...
<p>
<textarea name="t" style="width: 600px; height: 400px;" wrap="off"><?php if ($_POST['t']) {
    echo htmlspecialchars(trim($_POST['t']));
} ?></textarea>
</p>
