...
while (true) {
    ...
    foreach ($rows as $row) {
        ...
        if ($http_response_code!=200 && $http_response_code!=404) {
            echo 'musicbrainz unexpected response code '. $http_response_code.', wait 5 seconds then skip.'.PHP_EOL;
            sleep(5);
            continue;
        } else {
            echo 'musicbrainz http response code '.$http_response_code.PHP_EOL;
        }
        curl_close($ch);
