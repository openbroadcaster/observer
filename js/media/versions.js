    ...
    OB.API.post("media", "versions", data, function (response) {
        ...
        $.each(response.data.versions, function (index, version) {
            ...
            $tr.append(
                $("<td />").html(
                    version.created == 0
                        ? OB.t("Original Version") + "<br />" + format_timestamp(media.created)
                        : format_timestamp(version.created),
                ),
            );
            ...
            if (version.active == 0)
                $td.append(
                    $('<button data-t class="add">Activate</button>').click(function () {
                        OB.Media.versionSet(data.media_id, version.created);
                    }),
                );
            //T Edit
            $td.append(
                $('<button class="edit" data-t>Edit</button>').click(function () {
                    OB.Media.versionEdit(data.media_id, version);
                }),
            );
            //T Delete
            if (version.created != 0 && version.active == 0)
                $td.append(
                    $('<button class="delete" data-t>Delete</button>').click(function () {
                        OB.Media.versionDelete(data.media_id, version.created);
                    }),
                );
            //T Download
            $td.append(
                $("<button data-t>Download</button>").click(function () {
                    OB.Media.download(data.media_id, version.created);
                }),
            );
            $tr.append($td);
...
OB.Media.versionSet = function (media_id, created, confirm) {
    if (!confirm) {
        ...
        //T Cancel
        OB.UI.confirm(
            "Are you sure you want to activate this media version?",
            function () {
                OB.Media.versionSet(media_id, created, 1);
            },
            "Activate",
            "Cancel",
            "add",
...
OB.Media.versionDelete = function (media_id, created, confirm) {
    if (!confirm) {
        ...
        //T Cancel
        OB.UI.confirm(
            "Are you sure you want to delete this media version?",
            function () {
                OB.Media.versionDelete(media_id, created, 1);
            },
            "Delete",
            "Cancel",
            "delete",
