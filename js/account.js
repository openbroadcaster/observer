...
OB.Account.settings = function () {
    ...
    OB.API.multiPost(post, function (data) {
        ...
        if (languages && languages.data)
            $.each(languages.data, function (value, language) {
                $("#account_language").append(
                    '<option value="' + language.code + '">' + htmlspecialchars(language.name) + "</option>",
                );
            });

        if (themes && themes.data)
            $.each(themes.data, function (value, theme) {
...
OB.Account.settingsSubmit = function () {
    ...
    $("#account_appkey_table tbody tr").each(function (index, row) {
        data["appkeys"].push([$(row).attr("data-id"), $(row).find(".account_appkey_name").val()]);
    });
...
OB.Account.keyAdd = function () {
    OB.API.post("account", "key_new", {}, function (response) {
        ...
        $tr.append(
            $("<td/>").html(
                '<button onclick="OB.Account.keyPermissionsOpen(this);">Permissions</button><button class="delete" onclick="OB.Account.keyDelete(this);">Delete</button>',
            ),
        );
...
OB.Account.keyLoad = function () {
    OB.API.post("account", "key_load", {}, function (response) {
        ...
        $.each(response.data, function (index, row) {
            ...
            try {
                let permissions_v2 = JSON.parse(row.permissions_v2)
