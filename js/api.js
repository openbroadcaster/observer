...
OB.API.post = function (controller, action, sdata, callback_function, mode) {
  ...
  if (OB_API_REWRITE) var requestData = { "d": $.toJSON(sdata), "i": readCookie('ob_auth_id'), "k": readCookie('ob_auth_key') };
  else var requestData = { "c": controller, "a": action, "d": $.toJSON(sdata), "i": readCookie('ob_auth_id'), "k": readCookie('ob_auth_key') };
