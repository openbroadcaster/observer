...
OB.Player.savePlayer = function () {
    ...
    if ($("#player_settings_playlist").val().length != 0)
        post.default_playlist = $("#player_settings_playlist").val()[0];
    ...
    OB.API.post("players", "search", {}, function (data) {
        ...
        $.each(players, function (index, item) {
            ...
            if (
                ...
                OB.Settings.permissions.indexOf("view_player_monitor:" + item.id) == -1
            )
                return;
            $("#monitor_player_select").append(
...
OB.Player.monitorFilterAdd = function () {
    ...
    var $html = $(
        '<div><a href="javascript: OB.Player.monitorFilterDel(' +
            OB.Player.monitor_filter_id +
            ');">[x]</a> ' +
            htmlspecialchars(filter_friendly_string) +
            "</div>",
    );
...
OB.Player.monitorSearch = function (result_type = "json") {
    ...
    OB.API.post("players", "monitor_search", fields, function (data) {
        ...
        if (result_type === "csv" && total_rows > 0) {
            var data = new Blob([data.data.results], {
