...
OB.User.manageUsersGroupList = function (callback) {
    OB.API.post("users", "group_list", {}, function (data) {
        ...
        $.each(groups, function (index, group) {
            var html =
                '<div><input type="checkbox" value="' + group.id + '"> ' + htmlspecialchars(group.name) + "</div>";
    ...
    OB.User.manageUsersGroupList(function () {
        $user
            ...
            .each(function (index, element) {
                $("#user_addedit_group_list input[value=" + $(element).attr("data-group_id") + "]").attr(
                    "checked",
                    true,
                );
            });
...
OB.User.manageUsersSave = function () {
    ...
    $("#user_appkey_table tbody tr").each(function (index, row) {
        fields.appkeys.push([$(row).attr("data-id"), $(row).find(".user_appkey_name").val()]);
    });
...
*/
        ...
        OB.UI.confirm(
            ...
            function () {
                OB.User.manageUsersDelete(true);
            },
            "Yes, Delete",
            "No, Cancel",
            "delete",
...
OB.User.manageUsersKeyAdd = function () {
    OB.API.post("users", "user_manage_key_new", { id: $("#user_addedit_id").val() }, function (response) {
        ...
        $tr.append(
            $("<td/>").html(
                '<button class="delete" class="user_appkey_delete" onclick="OB.User.manageUsersKeyDelete(this);">Delete</button>',
            ),
        );
        ...
        OB.API.post("users", "group_list", {}, function (groups) {
            ...
            if (data.status != false) {
                ...
                $.each(categories, function (category, permissions) {
                    //T player
                    if (category.match(/^player: /))
                        var category_translated = category.replace(/^player: /, OB.t("player") + ": ");
                    else var category_translated = category; // no dynamic variable translation for now

                    $("#permissions_table tbody").append(
                        '<tr class="permission-category" ><th colspan="1000">' +
                    ...
                    $.each(permissions, function (index, permission) {
                        $("#permissions_table tbody").append(
                            '<tr data-permission="' +
                                ...
                                '</td><td class="center"><span class="checkmark">&#10003;</span></td></tr>',
                        );
                        ...
                        $.each(groups, function (index, group) {
                            ...
                            $(
                                '#permissions_table tbody tr[data-permission="' +
                                    htmlspecialchars(permission.name) +
                                    '"]',
                            ).append("<td >" + check + "</td>");
                        });
...
OB.User.managePermissionsDelete = function (confirm) {
    if (confirm) {
        ...
        OB.UI.confirm(
            ...
            function () {
                OB.User.managePermissionsDelete(true);
            },
            "Yes, Delete",
            "No, Cancel",
            "delete",
...
OB.User.managePermissionsSave = function () {
    ...
    $("#permissions_addedit_form .permission_checkbox").each(function (index, element) {
        if ($(element).prop("checked") == true) fields.permissions.push($(element).attr("data-name"));
    });
...
OB.User.managePermissionsForm = function (id) {
    ...
    $.each(OB.User.manage_permissions_list, function (category, permissions) {
        ...
        var $fieldset = $(
            "<fieldset><legend data-t >" + htmlspecialchars(category_translated) + "</legend></fieldset>",
        );
        ...
        $.each(permissions, function (index, permission) {
            ...
            $fieldrow.append(
                "<label data-t>" +
                    htmlspecialchars(permission.description) +
                    "</label>" +
                    '<input class="permission_checkbox" data-name="' +
                    permission.name +
                    '" type="checkbox"> ',
            );
