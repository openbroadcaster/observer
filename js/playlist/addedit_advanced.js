            ...
            if ($(ui.draggable).attr("data-mode") == "media") {
                ...
                $(".sidebar_search_playlist_selected").each(function (index, element) {
                    OB.API.post("playlists", "get", { id: $(element).attr("data-id") }, function (data) {
                        ...
                        //T A media item is marked as private. It can only be used in playlists created by the same owner.
                        if (not_supported_error && private_media_alert)
                            OB.UI.alert(
                                OB.t(
                                    "Dynamic selections and station IDs are not supported by advanced playlists. These items have been ignored.",
                                ) +
                                    "\n\n" +
                                    OB.t(
                                        "A media item is marked as private. It can only be used in playlists created by the same owner.",
...
OB.Playlist.advancedItemsDisplay = function () {
    if (!OB.Playlist.advanced_items.length) {
        $("#playlist_edit_advanced_help").show();
        $("#playlist_edit_advanced_times").hide();
    } else {
        $("#playlist_edit_advanced_help").hide();
        $("#playlist_edit_advanced_times").show();
    }
    ...
    $.each(OB.Playlist.advanced_items, function (index, item) {
        ...
        if (height > 0) {
            $("#playlist_edit_advanced_items").append(
                '<div class="playlist_addedit_advanced_item ' +
                    item.type +
                    '" data-id="' +
                    index +
                    '" id="playlist_addedit_advanced_item_' +
                    index +
                    '" style="left: ' +
                    left +
                    "px; width: " +
            ...
            $("#playlist_addedit_advanced_item_" + index).dblclick(function (e) {
                OB.Playlist.addeditItemProperties(index, item.type);
            });
...
OB.Playlist.advancedZoomOut = function () {
    ...
    if (selected_id)
        $("#playlist_edit_advanced .playlist_addedit_advanced_item[data-id=" + selected_id + "]").addClass("selected");
};
...
OB.Playlist.advancedZoomWheel = function (event, delta) {
    if (delta > 0) {
        OB.Playlist.advancedZoomIn();
        event.preventDefault();
        return false;
    } else if (delta < 0) {
        OB.Playlist.advancedZoomOut();
        event.preventDefault();
        return false;
    }
};
...
OB.Playlist.advancedTimes = function () {
    ...
    while (present_time <= OB.Playlist.advanced_last_position) {
        $("#playlist_edit_advanced_times").append(
            '<div class="playlist_edit_advanced_time" style="top: ' +
                OB.Playlist.advancedItemOffset(present_time) +
                'px">' +
                secsToTime(present_time) +
                "</div>",
        );
        present_time += time_increment;
    }

    var required_cell_height =
        $("#playlist_edit_advanced_times div").last().position().top +
        $("#playlist_edit_advanced_times div").last().height();
