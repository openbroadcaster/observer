        ...
        drop: function (event, ui) {
            ...
            if ($(ui.draggable).attr("data-mode") == "media") {
                $(".sidebar_search_media_selected").each(function (index, element) {
                    if (
                        $(element).attr("data-visibility") == "private" &&
                        $(element).attr("data-owner_id") != playlist_owner_id
                    ) {
                        private_media_alert = true;
                        return true;
                    }

                    if ($(element).attr("data-type") == "image" || $(element).attr("data-type") == "document")
                        var insert_duration = 15;
                ...
                if (private_media_alert)
                    OB.UI.alert(
                        "A media item is marked as private. It can only be used in playlists created by the same owner.",
                    );
                ...
                $(".sidebar_search_playlist_selected").each(function (index, element) {
                    OB.API.post("playlists", "get", { id: $(element).attr("data-id") }, function (data) {
                        ...
                        $.each(playlist_data["items"], function (index, item) {
                            ...
                            else if (item.type == "voicetrack") OB.Playlist.addeditInsertVoicetrack(item);
...
OB.Playlist.addeditKeypress = function (e) {
    ...
    if (
        !$("#playlist_edit_standard_container .playlist_addedit_item.selected").length &&
        !$("#playlist_edit_standard_container .playlist_addedit_liveassist_item.selected").length
    )
        return; // make sure we have something selected.
...
OB.Playlist.addeditImageDurationUpdate = function (id) {
    $("#playlist_addedit_item_" + id + " .playlist_addedit_duration").text(
        secsToTime($("#playlist_addedit_item_" + id).attr("data-duration")),
    );
    OB.Playlist.addeditTotalDuration();
};
...
OB.Playlist.addeditTotalDuration = function () {
    ...
    $("#playlist_items .playlist_addedit_item").each(function (index, element) {
        ...
        if ($(element).attr("data-type") == "dynamic" || $(element).attr("data-type") == "station_id")
            is_estimated = true;
    });
...
OB.Playlist.addeditInsertDynamic = function (
    is_new,
    query,
    duration,
    selection_name,
    ...
    $("#playlist_items").append(
        '<div class="playlist_addedit_item" id="playlist_addedit_item_' +
            OB.Playlist.addedit_item_last_id +
            '"><span class="playlist_addedit_thumbnail"></span><span class="playlist_addedit_description">' +
            htmlspecialchars(OB.t("Dynamic Selection")) +
            ': <span id="playlist_dynamic_selection_' +
            OB.Playlist.addedit_item_last_id +
            '_name"></span></span><span class="playlist_addedit_duration" id="playlist_dynamic_selection_' +
            OB.Playlist.addedit_item_last_id +
            '_duration"></span></div>',
    );
...
OB.Playlist.addeditInsertVoicetrack = function (item) {
    OB.Playlist.addedit_item_last_id += 1;
    ...
    if (!item) {
        ...
        $("#playlist_addedit_item_" + OB.Playlist.addedit_item_last_id).attr("data-id", item.id);
        ...
        if (properties["voicetrack_volume"]) {
            $("#playlist_addedit_item_" + OB.Playlist.addedit_item_last_id).attr(
                ...
                properties["voicetrack_volume"] * 100,
            );
        }
        ...
        if (properties["voicetrack_fadeout_before"]) {
            $("#playlist_addedit_item_" + OB.Playlist.addedit_item_last_id).attr(
                ...
                properties["voicetrack_fadeout_before"],
            );
        }
        ...
        if (properties["voicetrack_fadein_after"]) {
            $("#playlist_addedit_item_" + OB.Playlist.addedit_item_last_id).attr(
                ...
                properties["voicetrack_fadein_after"],
            );
        }
    }
...
OB.Playlist.addeditGetItems = function () {
    ...
    $("#playlist_items")
        ...
        .each(function (index, element) {
            ...
            if ($(element).attr("data-type") == "dynamic")
                items.push({
                    type: "dynamic",
            ...
            else if ($(element).attr("data-type") == "custom")
                items.push({
                    type: "custom",
                ...
                items.push({
                    ...
                    voicetrack_volume: $(element).attr("data-voicetrack_volume") / 100,
